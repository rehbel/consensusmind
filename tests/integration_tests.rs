...
async fn test_metadata_store() -> Result<()> {
    ...
    assert!(PathBuf::from(&filepath).exists(), "PDF file should exist");
    assert_eq!(store.count_downloaded(), 1, "One paper should be marked downloaded");

    assert!(store.is_downloaded(arxiv_id), "Paper should be marked as downloaded");

    let retrieved = store.get_paper(arxiv_id);
    assert!(retrieved.is_some(), "Should retrieve paper metadata");
    assert!(retrieved.unwrap().pdf_path.is_some(), "PDF path should be set");
